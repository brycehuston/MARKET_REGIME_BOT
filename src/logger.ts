...
export function logSnapshot(
  ...
  appendLine(config.paths.snapshotJsonl, JSON.stringify({
    ...
    ...liquidityRotationTelemetry,
      retBtc1h: accuracyFields?.retBtc1h ?? null,
      retEth1h: accuracyFields?.retEth1h ?? null,
      retSol1h: accuracyFields?.retSol1h ?? null,
    eventContext,
    calendarContext: eventContext?.calendarContext,
    holidayContextV1: eventContext?.holidayContextV1,
    launchWindowContext: eventContext?.launchWindowContext,
    weekendFlag: eventContext?.calendarContext?.weekendFlag,
    monthEndFlag: eventContext?.calendarContext?.monthEndFlag,
    quarterEndFlag: eventContext?.calendarContext?.quarterEndFlag,
    yearEndFlag: eventContext?.calendarContext?.yearEndFlag,
    longWeekendFlag: eventContext?.calendarContext?.longWeekendFlag,
    calendarLiquidityContext: eventContext?.calendarContext?.liquidityContext,
    activeHolidayNames: eventContext?.holidayContextV1?.activeHolidays.map((holiday) => holiday.name),
    holidayCountryCodes: eventContext?.holidayContextV1?.countryCodes,
    launchWindowActive: eventContext?.launchWindowContext?.launchWindowActive,
    launchWindowType: eventContext?.launchWindowContext?.launchWindowType,
    launchWindowName: eventContext?.launchWindowContext?.launchWindowName,
    launchWindowRisk: eventContext?.launchWindowContext?.launchWindowRisk,
    launchWindowReason: eventContext?.launchWindowContext?.launchWindowReason,
    calendarContextVersion: eventContext?.calendarContext?.calendarContextVersion,
